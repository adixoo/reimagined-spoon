  ...
  > {
    ...
    while (run) {
      for (let cycle = 1; cycle <= CYCLES; cycle++) {
        ...
        // If both scores are 0, nothing is matching — keep waiting in the for loop
        if (successPct === 0 && topFlowPct === 0) {
          this.log(`⏳ Both scores are 0 — waiting ${PAUSE_MS}ms`);
        ...
        if (topFlow && topFlowPct > successPct) {
          ...
          try {
            for (const cmd of step.commands) {
              const sel =
                "selector" in cmd ? ` [${(cmd as any).selector}]` : "";
      ...
      // 50 cycles exhausted without success or failure — stop the while loop
      run = false;
    }
